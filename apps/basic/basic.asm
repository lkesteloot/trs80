...
; Macro to negate a 16-bit value. Pass the high and low registers.
	...
	.org 0x0066
	; TODO the Model III reset button goes here, should reset the ROM.
	retn
...
find_nul:				; Find the nul end-of-line byte.
	...
	inc de
	jp nz,find_nul
	ld (hl),e			; Write pointer to next line.
	inc hl
	ld (hl),d
	ld hl,de
	jr loop
...
keyboard_matrix_shifted:
	db "`ABCDEFGHIJKLMNOPQRSTUVWXYZ     _!", 34, "#$%&'()*+<=>?", NL, 0, BREAK, 0, 0, 8, 9, 32
tokens:
