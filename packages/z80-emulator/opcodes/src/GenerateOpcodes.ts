...
function generateDispatch(pathname: string, prefix: string): string {
    const output: string[] = [];
    addLine(output, "");
    addLine(output, "// The contents of this map are auto-generated by GenerateOpcodes.ts.");
    addLine(output, "");
    ...
    fs.readFileSync(pathname, "utf-8").split(/\r?\n/).forEach((line: string) => {
        ...
        addLine(output, "fillMap.set(0x" + toHex(number, 2) + ", (z80: Z80) => { // " + ((opcode || "") + " " + (params || "")).trim());
        enter();
        ...
        for (const fallthroughOpcode of fallthroughOpcodes) {
            addLine(output, "fillMap.set(0x" + toHex(fallthroughOpcode, 2) + ", fillMap.get(0x" + toHex(number, 2) + ") as (z80: Z80) => void);");
        }
        fallthroughOpcodes = [];
    });
